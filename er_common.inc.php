...
function show_er($db, $context = "search_er.php" ) {
   ...
   $_SESSION["search_query"] =
      "SELECT er.draft_number, er.date, er.status, section.name, er.created_by, er.section, er.description"
      . " FROM er, section"
      . " WHERE er.section=section.id"
      . " AND er.date>='$from_date'"
      . " AND er.date<='$to_date'";
   ...
   switch ( $context ) {
	   ...
	   if ($created_by == "anybody") {
	      ...
	      if (($user_role == $cfg["registrar"]) || ($user_role == $cfg["finofficer"]) || ($user_role == $cfg["finmember"])) {
	         $_SESSION["search_query"] .= " AND ( er.created_by LIKE '$created_by_query' )";
	      }else{
	         $_SESSION["search_query"] .= " AND ( er.created_by LIKE '$created_by_query' AND $section_priviledge )";
	      }
	   } else if ( ( $created_by == "") || ( $created_by == $cfg[ "sysadmin_email" ] ) ) {
	      $created_by_query= $username;
	      $_SESSION["search_query"] .= " AND ( er.created_by LIKE '$created_by_query' OR  $section_priviledge )";
...
/*    $_SESSION["search_query"] .=
        " AND ( er.er_approved_number LIKE '0_____'"
      . " 	OR er.er_approved_number LIKE ''"
      . " 	OR er.er_approved_number IS NULL )";*/

   if ( ($action == "search_section") || ( isset($section_id ) ) ) {
   	if ( $section_id != "1" )
	      $_SESSION["search_query"] .= " AND er.section=$section_id";
   }
   if ( isset($status) && ( $status != "none" ) && ( $status != "" ) && ( $status != "any" )) {
	      $_SESSION["search_query"] .= " AND er.status='$status'";
   }
...
//
function get_paint_er_table_header($db, $context = "search_er.php" ) {
  global $username, $user_role, $cfg, $section_id, $vendor_id, $status, $created_by;
		...
		$status_filter = "<select name=\"status\" onchange=\"javascript:if (this.value){window.location='$context?$status_filter_query&status='+this.value+'';}\">";
                $selected_none =  $selected_open =  $selected_requested =  $selected_approved =  $selected_close =  $selected_cancel = "";
                ...
                $status_filter .= "<option value=\"Open\" $selected_open class=\"er_open\">Open</option>";
                $status_filter .= "<option value=\"Requested\" $selected_requested class=\"er_requested\">Requested</option>";
                $status_filter .= "<option value=\"Approved\" $selected_approved class=\"er_approved\">Approved</option>";
              	$status_filter .= "<option value=\"Closed\" $selected_close class=\"er_closed\">Closed</option>";
              	$status_filter .= "<option value=\"Canceled\" $selected_cancel class=\"er_canceled\">Canceled</option>";
	...
	switch ($context) {
		case "search_er.php" :
	 echo "<td align=\"center\">". $SortLinkDescBegin . "er.draft_number" . $SortLinkDescEnd . " <b>Req.</b> "            . $SortLinkAscBegin . "er.draft_number" . $SortLinkAscEnd . "</td>";
         ...
         echo "<td align=\"center\">". $SortLinkDescBegin . "er.date"         . $SortLinkDescEnd . " <b>Date</b> "            . $SortLinkAscBegin . "er.date"         . $SortLinkAscEnd . "</td>";
         ...
         echo "<td align=\"center\">". $SortLinkDescBegin . "er.description"    . $SortLinkDescEnd . " <b>Description</b> "         . $SortLinkAscBegin . "er.description"    . $SortLinkAscEnd . "</td>";
         echo "<td align=\"center\">". $SortLinkDescBegin . "er.status"         . $SortLinkDescEnd . " <b>$status_filter</b> "      . $SortLinkAscBegin . "er.status"         . $SortLinkAscEnd . "</td>";
         //echo "<td align=\"center\">". $SortLinkDescBegin . "er.er_approved_number"         . $SortLinkDescEnd . " <b>TR</b> "            . $SortLinkAscBegin . "er.er_approved_number"         . $SortLinkAscEnd . "</td>";
         echo "<td align=\"right\">". "<b>Requested</b>" ."</td>";
         echo "<td align=\"right\">". "<b>Prepaid</b>" ."</td>";
...
// Items
// Comment
function er_description($draft_number) {
	global $db;
	...
	$txt .= str_pad("Date", 15) . ": " . $er->fields["date"] . "\r\n";
	$txt .= str_pad("Funding", 15) . ": " . $section->fields["name"] ."\r\n";
	$txt .= str_pad("Description", 15). ": " . $er->fields["description"] . "\r\n";
	$txt .= str_pad("Status", 15). ": " . $er->fields["status"] . "\r\n";
	$txt .= str_pad("Requested By", 15). ": " . $cr_user->fields["fullname"] . "\r\n";

	if (($er->fields["status"] == "Approved") || ($er->fields["status"] == "Closed")) {
		$app_user	= $db->Execute("SELECT fullname FROM users WHERE username='" . $er->fields["approved_by"] . "'");
		$txt .= str_pad("Approved by", 15) . ": " . $app_user->fields["fullname"] . "\r\n";
		/*if ( $er->fields["er_approved_number"] != "" ) {
			$txt .= str_pad("TR Number", 15) . ": " . $er->fields["er_approved_number"] . "\r\n";
		}*/
	}
	...
	$txt .= str_pad("", 120, " ") . "\r\n";
	$txt .= str_pad("Receipt", 10) . str_pad("Type", 20) . str_pad("Description", 60) . str_pad("Price", 10, " ", STR_PAD_LEFT) . str_pad("Prepaid", 9, " ", STR_PAD_BOTH) . str_pad("Comment", 40) .  "\r\n";
	$txt .= str_pad("", 120, "-") . "\r\n";

	while (!$line_items->EOF) {
	        $txt .= str_pad($line_items->fields["receipt"], 10);
	...
	$txt .= str_pad("", 120, "-") . "\r\n";
	$txt .= str_pad("Total", 80) . str_pad(get_er_total($draft_number), 10,  " " , STR_PAD_LEFT) ." ". get_currency_sign( 'e', "text" ).  "\r\n";

	return $txt;
}

	
function get_er_total( $draft_number ) {

   global $db;

   if (!$er = $db->Execute("SELECT * FROM er WHERE draft_number=$draft_number")) {
	echo "<table class=\"warn_db\" width=\"100%\"><tr><td>Cannot get ER total | DB ERROR: " . $db->ErrorMsg() . "</td></tr></table>";
	exit;
   }
   ...
   $er_total = 0;
   while (!$line_items->EOF) {
	$er_total += $line_items->fields["price"] * $line_items->fields["quantity"] * $line_items->fields["exchangerate"];
	$line_items->MoveNext();
   }

   return sprintf("%01.2f", $er_total);
}

function get_er_prepaid( $draft_number ) {
   ...
   while (!$line_items->EOF) {
	if ( $line_items->fields["prepaid"] == "Y" ) {
		$er_total += $line_items->fields["price"] * $line_items->fields["exchangerate"];
	}
	$line_items->MoveNext();
   }
...
function get_er_advance( $draft_number ) {
   ...
   if ($er->fields["tr_draft_number"] != "") {
	...
	$tr = $db->Execute("SELECT * FROM tr WHERE draft_number=" . $er->fields["tr_draft_number"]);

	$er_total = $tr->fields["advance_transfered"];
   }
...
function get_er_currency_sign( $draft_number, $symboletype = "html" ) {
   global $db;
  ...
  //return get_currency_sign( $er->fields["currency"], $symboletype);
